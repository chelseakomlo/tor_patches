...
crypto_pk_get_hashed_fingerprint(crypto_pk_t *pk, char *fp_out)
{
  ...
  if (crypto_digest(hashed_digest, digest, DIGEST_LEN) == -1) {
    return -1;
  }
                 ...
                 digest_algorithm_t algorithm)
{
  ...
  if (algorithm == DIGEST_SHA256)
    is_err = SHA256((const uint8_t*)m,len,(uint8_t*)digest) == NULL;
  else
    is_err = sha3_256((uint8_t *)digest, DIGEST256_LEN,(const uint8_t *)m, len)
            == -1;
                               ...
                               uint8_t *key_out, size_t key_out_len)
{
  ...
  for (cp = key_out, i=0; cp < key_out+key_out_len;
       ++i, cp += DIGEST_LEN) {
    tmp[key_in_len] = i;
    if (crypto_digest((char*)digest, (const char *)tmp, key_in_len+1) == -1)
      goto exit;
